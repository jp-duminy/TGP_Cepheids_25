...
# ============================================================
# Andromeda filename pattern (extracts filter: B or V)
# ============================================================

ANDROMEDA_PATTERN = re.compile(
    r"Andromeda.*Filter_([BV])_00_.*\.fits",
    re.IGNORECASE
)

# ============================================================
# Utility: select longest-exposure FITS file
# ============================================================

def select_longest_exposure(files):
    """
    From a list of FITS files, return the one with the longest EXPTIME.
    """
    best_file = None
    best_exp = -1
    ...
    for f in files:
        with fits.open(f) as hdul:
            exp = hdul[0].header.get("EXPTIME", 0)
            if exp > best_exp:
                best_exp = exp
                best_file = f

    return best_file, best_exp
    
# ============================================================
# Utility: find Andromeda images by filter
# ============================================================

def find_andromeda_images(directory):
    images = {'B': [], 'V': []}

    for f in Path(directory).glob("*.fits"):
        match = ANDROMEDA_PATTERN.search(f.name)
        if match:
            filt = match.group(1).upper()
            images[filt].append(f)
...
def reduce_andromeda(
    ...
    """
    Reduce Andromeda images in B and V.
    Chooses longest exposure image per filter.
    """
    ...
    print("\n" + "="*60)
    print("Reducing Andromeda Images")
    print("="*60)

    images_by_filter = find_andromeda_images(science_dir)

    for filt, files in images_by_filter.items():
        if len(files) == 0:
            print(f"No {filt}-band images found")
            continue

        # Get correct calibration set for this filter
        calib_name = f"{WEEK_NAME}_{filt}_binning1x1"
        calib_set = calib_mgr.calibration_sets[calib_name]
        reducer = Cepheid_Image_Reducer(calib_set)

        chosen_file, exp = select_longest_exposure(files)

        print(f"\nFilter {filt}:")
        print(f"  Found {len(files)} images")
        print(f"  Selected: {chosen_file.name} ({exp:.1f}s)")

        reduced, header = reducer.perform_reduction(chosen_file)

        header['FILTER'] = filt
        header['OBJECT'] = 'Andromeda'
        header['COMMENT'] = 'Reduced single image (no stacking)'

        save_path = output_dir / f"Andromeda_{filt}_reduced.fits"
        fits.PrimaryHDU(reduced, header=header).writeto(
...
if __name__ == "__main__":
    ...
    CALIB_DIR = f"{BASE_DIR}/Calibrations"
    SCIENCE_DIR = f"{BASE_DIR}/Cepheids/2025-10-06"
    OUTPUT_DIR = f"{BASE_DIR}/student-work/Andromeda/reduced"

    NIGHT_NAME = "2025-10-06"
    WEEK_NAME = "week3"

    # ------------------------------------------------------------
    # Set up calibration manager
    # ------------------------------------------------------------
    calib_mgr = Calibration_Manager(CALIB_DIR)

    # Create calibration sets for BOTH filters
    calib_mgr.week_calibrations(WEEK_NAME, filter="B")
    calib_mgr.week_calibrations(WEEK_NAME, filter="V")

    # Map this night to the calibration week
   #calib_mgr.map_night_to_week(NIGHT_NAME, WEEK_NAME)
    calib_mgr.map_night_to_week(NIGHT_NAME, f"{WEEK_NAME}_V_binning1x1")

    # Prepare master bias & flats
    calib_mgr.prepare_all()

    # ------------------------------------------------------------
    # Reduce Andromeda (no stacking)
    # ------------------------------------------------------------
    reduce_andromeda(
